...
Item {
    ...
    Item {
        ...
        Column {
            ...
            Rectangle {
                ...
                function findUser() {
                    if (typeof userModel === "undefined" || typeof userModel.rowCount !== "function") return false
                    var count = userModel.rowCount()
                    if (count === 0) return false
                    if (selectedIndex >= 0 && selectedIndex < count) return true
                    // R1999_2 approach: use lastIndex
                    if (typeof userModel.lastIndex !== "undefined" && userModel.lastIndex >= 0 && userModel.lastIndex < count) {
                        selectedIndex = userModel.lastIndex
                        return true
                    }
                    // Fallback: match by name
                    for (var i = 0; i < count; i++) {
                        var item = userModel.get ? userModel.get(i) : null
                        if (item && item.name === sddm.lastUser) {
                            selectedIndex = i
                            return true
                        }
                    }
                ...
                Component.onCompleted: findUser()
                ...
                Text {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    text: userHelper.currentItem ? userHelper.currentItem.uName : "Username"
                    color: userHelper.currentItem ? "#c0caf5" : "#565f89"
            ...
            Rectangle {
                ...
                function findSession() {
                    if (typeof sessionModel === "undefined" || typeof sessionModel.rowCount !== "function") return false
                ...
                Component.onCompleted: findSession()
            ...
            Rectangle {
                ...
                property string btnText: "Login"

                Text {
                    anchors.centerIn: parent
                    text: loginBtn.btnText
                    color: "#1a1b26"
                    font.pointSize: 15 * scaleFactor
                    font.family: "M PLUS Rounded 1c", "Noto Sans JP", "sans-serif"
                    font.weight: Font.Bold
                ...
                Timer {
                    id: bootTimer
                    interval: 800
                    onTriggered: sddm.login(userHelper.currentItem ? userHelper.currentItem.uName : "", passwordInput.text, sessionPill.currentIndex)
                }
            }
    ...
    ListView {
        id: userHelper
        model: typeof userModel !== "undefined" ? userModel : null
        currentIndex: userPill.selectedIndex
    ...
    ListView {
        id: sessionHelper
        model: typeof sessionModel !== "undefined" ? sessionModel : null
        currentIndex: sessionPill.currentIndex
        opacity: 0; width: 1; height: 1; z: -100
        delegate: Item {
    ...
    Connections {
        ...
        function onRowsInserted(parent, first, last) {
            if (userPill && typeof userPill.findUser === "function")
                userPill.findUser()
        }
    ...
    Connections {
        ...
        function onRowsInserted(parent, first, last) {
            if (sessionPill && typeof sessionPill.findSession === "function")
                sessionPill.findSession()
        }
        function onModelReset() {
            if (sessionPill && typeof sessionPill.findSession === "function")
                sessionPill.findSession()
    ...
    function login() {
        ...
        loginBtn.btnText = "Logging in..."
        errorText.text = ""
        if (userPill.selectedIndex === -1) {
            var count = typeof userModel !== "undefined" && userModel.rowCount ? userModel.rowCount() : 0
            if (count > 0) {
                userPill.selectedIndex = 0
            } else {
                errorText.text = "No users available"
